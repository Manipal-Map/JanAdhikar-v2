...
export default function IntakeChatView() {
  ...
  return (
    ...
    >
      <div className="w-full max-w-3xl mx-auto flex items-center justify-between py-3 border-b border-slate-300 mb-4 bg-white/95 backdrop-blur-md px-4 rounded-2xl shadow-sm relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-ashoka-navy border border-slate-800 flex items-center justify-center text-white shadow-sm">
              <ShieldCheck size={20} className="text-[#FF9933]" />
            </div>
            <div>
              <h1 className="text-base font-extrabold text-ashoka-navy tracking-tight">JanAdhikar Legal Intake</h1>
              <p className="text-xs text-slate-500 font-medium">Step 2: Fact &amp; Information Gathering</p>
            </div>
          </div>
        </div>
        {caseId && (
          <span className="text-xs font-mono font-bold text-slate-600 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-300 shadow-sm">
            #{caseId}
          </span>
        )}
      </div>
      ...
      <div className="w-full max-w-3xl flex-1 overflow-y-auto space-y-5 pr-1 mb-4 relative z-10">
          ...
          >
            ...
            <div className={`p-4 rounded-2xl max-w-[80%] text-sm leading-relaxed shadow-sm whitespace-pre-line text-left border ${
              msg.role === 'user' 
                ...
                : 'bg-white/95 backdrop-blur-md text-ashoka-navy border-slate-300 rounded-tl-none font-medium'
            }`}>
              {msg.content}
            </div>
          </motion.div>
        ...
        {loading && (
          <div className="flex items-center gap-3">
            ...
            <div className="p-4 bg-white/95 backdrop-blur-md border border-slate-300 rounded-2xl rounded-tl-none text-slate-500 text-sm flex items-center gap-2 shadow-sm font-medium">
      ...
      <div className="w-full max-w-3xl mx-auto space-y-3 relative z-10">
        {isReady && (
          <motion.div 
            ...
            className="p-4 bg-white/95 backdrop-blur-md border border-slate-300 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm"
          >
        ...
        <form onSubmit={handleSend} className="p-2 flex items-end gap-2 shadow-sm bg-white/95 backdrop-blur-md rounded-2xl border border-slate-300 focus-within:border-[#FF9933] focus-within:ring-1 focus-within:ring-[#FF9933] transition-all">
          <textarea
            rows={2}
            value={input}
